...
def download(output):
    if output.shape[0]>0:
        ...
        with col1:	
            ...
            st.write(f'<style>div.row-widget.stDataFrame > div:nth-child(1) > div:nth-child(1) {{text-align: center;}}</style>', unsafe_allow_html=True)
            st.write(output.reset_index(drop=True))
        with col2:
